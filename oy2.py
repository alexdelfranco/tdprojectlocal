...
def mockup_extract(dataframe, mockup_number):
  ...
  for i in range(len(dataframe[col_names['names']])):
    if dataframe[col_names['names']][i] == '':
      columns['length'] = i
      break
  # For each of the seven columns of data
...
def filename_extract(mockups):
  ...
  file_name_list = []
  for mockup_number in range(1,mnumber+1):
...
def update_plist(mockups):
  file_names = filename_extract(mockups)
  ...
  for folder in ['VLT','GEMINI','SUBARU']:
    ...
    images = get_image_paths('Original_Data/'+folder)
...
def preprocess_images(mockup,img_paths):
  '''
...
def process_images(mockup,path_list):
  '''
  Input: The main mockup dictionary (imported from a csv) and a dictionary of mockup images
  Output: A dictionary of mockup images with median subtracted backgrounds
  Description: Subtracts the median value of image backgrounds from the entire image to standardize background color
  '''
  # Generate preprocessed mockup images
  mockup_images = preprocess_images(mockup,path_list)
...
def plot_mockup(mockup,dimensions,colormap,norm_alpha,mockup_images,save_to_drive=False,save_path='null'):
  '''
